...
int A7105_calibrate_VCB(u8 channel) {
  ...
  calibration_result = A7105_ReadReg(A7105_24_VCO_CURCAL);
...
void A7105_WriteData(u8 *dpbuffer, u8 len, u8 channel)
{
  ...
  // 0x05 FIFO DATA register - about to send data to put into FIFO
  for (i = 0; i < len; i++)
    A7105_WriteReg(0x05, dpbuffer[i]); // send some data

  // set the channel
  A7105_WriteReg(0x0F, channel);
  A7105_Strobe(A7105_TX); // strobe command to actually transmit the data
